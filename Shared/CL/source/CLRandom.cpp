...
void cRandomMT::SetSeed(uint32_t seed)
{
    //
    ...
    for (int j = MT_N; j >= 0; j--)
    {
        *s++ = x;
        x *= 69069u; // if this is 64-bit, does it still mask down?
    }
}


uint32_t cRandomMT::Reload()
{
    uint32_t* p0 = mState;
    uint32_t* p2 = mState + 2;
    uint32_t* pM = mState + MT_M;
    ...
    for (s0 = mState[0], s1 = mState[1], j = MT_N - MT_M + 1; --j; s0 = s1, s1 = *p2++)
        *p0++ = *pM++ ^ (MIX_BITS(s0, s1) >> 1) ^ (LO_BIT(s1) ? MT_K : 0U);

    for (pM = mState, j = MT_M; --j; s0 = s1, s1 = *p2++)
        *p0++ = *pM++ ^ (MIX_BITS(s0, s1) >> 1) ^ (LO_BIT(s1) ? MT_K : 0U);
...
struct cSeed96
{
    ...
    cSeed96() : mX(123456789), mY(362436069), mZ(521288629) {}
};

uint32_t Shuffle96(cSeed96* seed)
{
    uint32_t t = seed->mX;
    t ^= t << 16;
    t ^= t >> 5;
