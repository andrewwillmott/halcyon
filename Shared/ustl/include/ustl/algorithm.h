...
int qsort_adapter (void* compareIn, const void* p1, const void* p2)
{
    const Compare& compare = *reinterpret_cast<Compare*>(compareIn);

    ConstPointer i1 = reinterpret_cast<ConstPointer>(p1);
    ConstPointer i2 = reinterpret_cast<ConstPointer>(p2);

    return (compare(*i1, *i2) ? -1 : (compare(*i2, *i1) ? 1 : 0));
}
...
void sort (RandomAccessIterator first, RandomAccessIterator last, Compare compare)
{
    ...
    qsort_r(first, last - first, sizeof(value_type), &compare, &qsort_adapter<const_pointer, Compare>);
}
