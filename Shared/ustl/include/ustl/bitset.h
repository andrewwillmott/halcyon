...
#ifndef USTL_BITSET_H
#define USTL_BITSET_H
...
namespace ustl
{
    ...
    private:
        inline value_type&      BitRef (uoff_t n)       { UASSERT(n < Size); return (m_Bits [n / s_WordBits]); }
        inline value_type       BitRef (uoff_t n) const { UASSERT(n < Size); return (m_Bits [n / s_WordBits]); }
        inline value_type       Mask (uoff_t n) const   { UASSERT(n < Size); return (1 << (n % s_WordBits)); }
    ...
    public:
        ...
        inline void             reset ()                    { fill_n (m_Bits, s_nWords, 0); }
        inline void             clear ()                    { fill_n (m_Bits, s_nWords, 0); }
        inline void             set ()                      { fill_n (m_Bits, s_nWords, -1); }
        inline bitset           operator~ () const          { bitset rv (*this); rv.flip(); return (rv); }
        inline size_type        size () const               { return (Size); }
        inline size_type        capacity () const           { return (s_nBits); }
        inline bool             test (uoff_t n) const           { return (BitRef(n) & Mask(n)); }
        inline bool             operator[] (uoff_t n) const     { return (test(n)); }
        inline const_iterator   begin () const              { return (m_Bits); }
        inline iterator         begin ()                    { return (m_Bits); }
        inline const_iterator   end () const                { return (m_Bits + s_nWords); }
        inline iterator         end ()                      { return (m_Bits + s_nWords); }

                            /// Returns the value_type with the equivalent bits. If size() > 1, you'll get only the first BitsInType(value_type) bits.
        inline value_type   to_value () const           { return (m_Bits[0]); }
                            /// Flips all the bits in the set.
        inline void         flip () { transform (begin(), end(), begin(), bitwise_not<value_type>()); }
        ...
        inline void         set (uoff_t first, uoff_t DebugArg(last), value_type v)
                            {
                                UASSERT(size_t (distance (first, last)) <= s_WordBits && "Bit ranges must be 32 bits or smaller");
                                UASSERT(first / s_WordBits == last / s_WordBits && "Bit ranges can not cross dword (4 byte) boundary");
                                UASSERT((v & BitMask(value_type,distance(first,last))) == v && "The value is too large to fit in the given bit range");
        ...
        inline bool         any () const        { value_type sum = 0; foreach (const_iterator, i, *this) sum |= *i; return (sum); }
        inline bool         none () const       { return (!any()); }
        inline size_t       count () const      { size_t sum = 0; foreach (const_iterator, i, *this) sum += popcount(*i); return (sum); }
        ...
        inline size_t       stream_size () const                { return (sizeof(m_Bits)); }
