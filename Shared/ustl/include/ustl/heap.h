...
public:
    ...
    inline size_type    size () const       { return (m_v.size()); }
    inline bool         empty () const      { return (m_v.empty()); }
    inline reference    top () const        { return (m_v.at(0)); }
    inline void         push (reference v)      { m_v.push_back (v); make_heap (m_v.begin(), m_v.end(), m_c); }
    inline void         pop ()              { pop_heap (m_v.begin(), m_v.end()); m_v.pop_back(); }
private:
