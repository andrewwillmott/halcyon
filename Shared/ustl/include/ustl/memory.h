...
#endif
#include "ustl/pair.h"
#include "ustl/iterator.h"
#include "ustl/limits.h"
...
public:
    ...
    inline pointer      release ()                  { pointer rv (m_p); m_p = NULL; return (rv); }
...
inline void construct (ForwardIterator first, ForwardIterator last)
{
    ...
    {
        ptrdiff_t bytesToClear = ((uint8_t*) last) - ((uint8_t*) first);
        if (bytesToClear > 0)   // memset crashes on -ve values.
            memset(first, 0, bytesToClear);
    }
    else
...
inline pair<T*, ptrdiff_t> make_temporary_buffer (void* p, size_t n, const T* ptype)
{
    ...
    /// \ingroup RawStorageAlgorithms
    #define UTSL_get_temporary_buffer(size, ptype)   make_temporary_buffer (alloca(size_of_elements(size, ptype)), size, ptype)
    #define UTSL_return_temporary_buffer(p)
#else
    #define UTSL_get_temporary_buffer(size, ptype)   make_temporary_buffer (malloc(size_of_elements(size, ptype)), size, ptype)
    #define UTSL_return_temporary_buffer(p)          if (p) free (p), p = NULL
#endif
