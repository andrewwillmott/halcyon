...
TESTFLIGHT_API_TOKEN="6bf46ed5f7f27c6dd0f0449f24096e1c_MzkzNTEzMjAxMi0wNC0xMSAxMDo1NjozNS44NDA3MTA"
#
# Find your TEAM_TOKEN at: https://testflightapp.com/dashboard/team/edit/
TESTFLIGHT_TEAM_TOKEN="22f3ff813525b4d508340483e367cd7e_Nzk5NDQyMDEyLTEyLTA3IDE0OjAyOjIzLjEyNjQxMw"
#
...
DISTRIBUTION_LISTS="DevTeam"
#
# Default selection of Distribution List(s), e.g. "DevTeam,Clients":
DISTRIBUTION_LISTS_DEFAULT_SELECTION="DevTeam"
#
...
#
SKIP_RESIGNING_AND_REPROVISIONING="NO"
#
# Uncomment this line to skip the Release Notes input step and to set a default value:
#
DEFAULT_RELEASE_NOTES="build pushed by upload-ios-archive.sh"
#
...
# Added by ajw...
# Specify this up front to avoid querying user whether to upload
SHOULD_UPLOAD="YES"
# Specify this to automatically select the matching identity rather than querying the user
DEFAULT_CODE_SIGN_IDENTITY="iPhone Distribution"
# Specify this up front to avoid querying user
SELECTED_PROFILE_NAME="Ad Hoc"
# Whether to skip notifying the user via xcode that everything finished
SKIP_DONE="YES"
...
# Enable these for testing without doing a clean rebuild each time =P
# CODE_SIGN_IDENTITY="iPhone Developer"
# WRAPPER_NAME=Fluid.app
# ARCHIVE_DSYMS_PATH="/Users/ajw/Library/Developer/Xcode/Archives/2013-07-06/Fluid_Develop 06-07-2013 23.09.xcarchive/dSYMs"
# ARCHIVE_PRODUCTS_PATH="/Users/ajw/Library/Developer/Xcode/Archives/2013-07-06/Fluid_Develop 06-07-2013 23.09.xcarchive/Products"
# DWARF_DSYM_FILE_NAME=Fluid.app.dSYM
...
if [ "$SKIP_RESIGNING_AND_REPROVISIONING" != "YES" ]; then
    ...
    if [ "$DEFAULT_CODE_SIGN_IDENTITY" = "" ]; then
        ...
        echo "Looking for $DEFAULT_CODE_SIGN_IDENTITY" >> $LOG
        CODE_SIGN_IDENTITY=`security find-identity -v -p codesigning | grep "$DEFAULT_CODE_SIGN_IDENTITY" | sed -e 's/^.* "//' -e 's/"$//'`
    fi
    ...
    do
        ...
        LANG=C sed -n '/<!DOCTYPE plist/,/<\/plist>/ p' \
